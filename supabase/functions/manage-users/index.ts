...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: callerProfile } = await adminClient.from('profiles').select('is_super_admin').eq('id', user.id).single();
    const isSuperAdmin = callerProfile?.is_super_admin === true;

    const { data: adminMemberships } = await adminClient.from('company_memberships').select('company_id').eq('user_id', user.id).eq('role', 'Admin');
    ...
    if (action === 'CREATE') {
      const { email, password, name, phone, company_id, role, is_super_admin } = payload;
    ...
    if (action === 'UPDATE') {
      const { id, email, password, name, phone, is_super_admin } = payload;
      
      if (!isSuperAdmin) {
        const { data: targetMemberships } = await adminClient.from('company_memberships').select('company_id').eq('user_id', id);
        const inSameCompany = targetMemberships?.some((tm: any) => adminMemberships.find((am: any) => am.company_id === tm.company_id));
      ...
      if (isSuperAdmin && is_super_admin !== undefined) {
        profileData.is_super_admin = is_super_admin;
      }
