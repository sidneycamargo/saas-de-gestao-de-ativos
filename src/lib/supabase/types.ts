    ...
    Tables: {
      ...
      contracts: {
        Row: {
          ...
          id: string
          identifier: string | null
          preventive_maintenance_period: string | null
      ...
      maintenance_assets: {
        ...
        Relationships: [
          {
            foreignKeyName: 'maintenance_assets_asset_id_fkey'
            columns: ['asset_id']
            isOneToOne: false
            referencedRelation: 'assets'
            referencedColumns: ['id']
          ...
          {
            foreignKeyName: 'maintenance_assets_maintenance_id_fkey'
...
//   identifier: text (nullable)
//   description: text (nullable)
//   preventive_maintenance_period: text (nullable)
// Table: groups
//   id: uuid (not null, default: gen_random_uuid())
//   company_id: uuid (not null)
//   name: text (not null)
...
// Table: maintenance_assets
//   FOREIGN KEY maintenance_assets_asset_id_fkey: FOREIGN KEY (asset_id) REFERENCES assets(id) ON DELETE CASCADE
//   UNIQUE maintenance_assets_maintenance_id_asset_id_key: UNIQUE (maintenance_id, asset_id)
//   FOREIGN KEY maintenance_assets_maintenance_id_fkey: FOREIGN KEY (maintenance_id) REFERENCES maintenances(id) ON DELETE CASCADE
//   PRIMARY KEY maintenance_assets_pkey: PRIMARY KEY (id)
// Table: maintenance_logs
//   FOREIGN KEY maintenance_logs_company_id_fkey: FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE CASCADE
//   FOREIGN KEY maintenance_logs_maintenance_id_fkey: FOREIGN KEY (maintenance_id) REFERENCES maintenances(id) ON DELETE CASCADE
//   PRIMARY KEY maintenance_logs_pkey: PRIMARY KEY (id)
//   FOREIGN KEY maintenance_logs_user_id_fkey: FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE SET NULL
// Table: maintenances
...
//   FOREIGN KEY maintenances_contract_id_fkey: FOREIGN KEY (contract_id) REFERENCES contracts(id) ON DELETE SET NULL
...
// Table: maintenance_assets
//   Policy "maintenance_assets_policy" (ALL, PERMISSIVE) roles={public}
//     USING: (EXISTS ( SELECT 1    FROM (maintenances m      JOIN company_memberships cm ON ((cm.company_id = m.company_id)))   WHERE ((m.id = maintenance_assets.maintenance_id) AND (cm.user_id = auth.uid()))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM (maintenances m      JOIN company_memberships cm ON ((cm.company_id = m.company_id)))   WHERE ((m.id = maintenance_assets.maintenance_id) AND (cm.user_id = auth.uid()))))
// Table: maintenance_logs
//   Policy "maintenance_logs_policy" (ALL, PERMISSIVE) roles={public}
//     USING: (EXISTS ( SELECT 1    FROM company_memberships cm   WHERE ((cm.company_id = maintenance_logs.company_id) AND (cm.user_id = auth.uid()))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM company_memberships cm   WHERE ((cm.company_id = maintenance_logs.company_id) AND (cm.user_id = auth.uid()))))
// Table: maintenances
...
// Table: maintenance_assets
//   CREATE UNIQUE INDEX maintenance_assets_maintenance_id_asset_id_key ON public.maintenance_assets USING btree (maintenance_id, asset_id)
// Table: maintenance_logs
//   CREATE INDEX idx_maintenance_logs_maintenance_id ON public.maintenance_logs USING btree (maintenance_id)
// Table: subscriptions
